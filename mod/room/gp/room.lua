---
--- Generated by EmmyLua(https://github.com/EmmyLua)
--- Created by dongyf.
--- DateTime: 2019-06-07 16:14
---

local RoomGP = class("RoomGP")
local tablex = require "pl.tablex"
local libcenter = require "libcenter"
local roomLogic = require("gp.logic")
local roomOnCall = require("gp.roomOnCall")

function RoomGP:roomInit(room_type)
    self._players = {}

    roomLogic:start()
end

function RoomGP:is_player_num_overload()
    return tablex.size(self._players) >= 10000
end

function RoomGP:initialize()
    RoomGP.onCall = roomOnCall
end

function RoomGP:broadcast(msg, filterUid)
    --DEBUG("broadcast")
    for k,v in pairs(self._players) do
        if not filterUid or filterUid ~= k then
            libcenter.send2client(k,msg)
        end
    end
end

local t
local idx

function RoomGP:enter(data)
    local uid = data.uid
    local player={
        uid=uid,
        agent=data.agent,
        node=data.node,
    }
    self._players[uid]=player
    self:broadcast({_cmd = "room_move.add", uid=uid,}, uid)

    --if self:is_player_num_overload() then
    --	t=timer:new()
    --	t:init()
    --	idx=t:register(3,self.gamestart,0,self)
    --	DEBUG("room player is overload")
    --end

    return SYSTEM_ERROR.success
end

function RoomGP:leave(uid)
    if not uid then
        ERROR("RoomGP leave uid is nil")
        return SYSTEM_ERROR.error
    end
    self._players[uid]=nil
    self:broadcast({_cmd = "movegame.leave", uid = uid}, uid)
    --DEBUG(t)
    --DEBUG(idx)
    --if t and idx then
    --    t:unregister(idx)
    --    DEBUG("unregister:"..idx)
    --end
    return SYSTEM_ERROR.success
end

return RoomGP